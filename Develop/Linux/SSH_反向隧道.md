...
- Develop
- Linux
last_modified: '2026-07-29T01:40:31Z'
---
...
# /etc/systemd/system/ssh-tunnel-18082.service        # 路径
systemctl daemon-reload                               # 修改或新建服务文件后，通知 systemd 重新加载配置
systemctl restart ssh-tunnel-18082.service            # 立即重启（或启动）该 SSH 隧道服务
systemctl enable ssh-tunnel-18082.service             # 将该服务设置为开机自启动
```

下面是将本机的 8082 端口反向映射到外网主机 mc3.en 的 18082 端口上（**-R [远程IP:]远程端口:本地IP:本地端口**）
 ```
[Unit]
...
ExecStart=/usr/bin/ssh -N \                           # -N 只建立隧道而不执行远程命令
  -R 18082:localhost:8082 \                           # 反向隧道配置：将远程主机的 18082 端口转发到本地的 8082 端口
  ...
  tunnel@mc3.en                                       # 连接的目标远程服务器用户名及 IP 地址
Restart=always                                        # 无论因何种原因退出（正常退出或报错），systemd 都会自动重启该服务
RestartSec=10                                         # 每次异常退出后，等待 10 秒再进行下一次重启尝试
[Install]
